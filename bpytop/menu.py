...
class MenuBar:
	...
	def __init__(self):
		...
		self.resized: bool = True
		self.menus: Dict[str, Dict[str, str]] = {}
		self.menu_length: Dict[str, int] = {}
		self.background: str = ""
...
class MainMenu(Menu):
	def __init__(latest_version):
		...
		while not cls.close:
			...
			if cls.resized:
				...
				if latest_version != VERSION:
					banner += f'{CursorChar.to(term.height, 1)}{Fx.b}{THEME.title}New release {latest_version} available at https://github.com/aristocratos/bpytop{Fx.ub}{term.fg}'
				cy = 0
			...
			if skip and redraw:
				self.window.view.draw_soon(out)
			elif not skip:
				self.window.view.draw_soon(f'{cls.background}{banner}{out}')
			skip = redraw = False
		...
		self.window.view.draw_soon(f'{Draw.saved_buffer()}')
		cls.background = ""
		cls.active = False
		cls.close = False
	...
	def __init__(self):
		...
		while not cls.close:
			...
			if skip and redraw:
				...
				self.window.view.draw_soon(f'{cls.background}{out_misc}{out}')
			skip = redraw = False
			...
			if key.input_wait(timer.left()):
				...
				if inputting:
					...
					if selected == "theme_background":
						...
						self.window.view.draw_soon(term.bg)
