...
class Controller:
	...
	def parse_input_key(self, input_key):
		...
		if input_key == ESCAPE_CODE:
			...
			try:
				self.mouse_pos = \
					(int(input_key.split(";")[1]), int(input_key.split(";")[2].rstrip("mM")))
			except:
				pass
			else:
				if input_key.startswith(
					...
					if Menu.active:
						...
						for key_name, positions in self.mouse.items():  # * Check if mouse position is clickable
...
class Terminal:
	...
	def __init__(self, width, height):
		...
		self.mouse_direct_on	= "\033[?1003h"							#* Enable reporting of mouse position at any movement
